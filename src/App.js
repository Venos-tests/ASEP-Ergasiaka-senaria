...
// Accepts "ΑΣΕΠ Unlocked 01" through "ΑΣΕΠ Unlocked 99"
function isValidAccessCode(code) {
  return /^ΑΣΕΠ Unlocked \d{2}$/.test(code.trim());
}
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{overflow:"visible"}}>
      {rings.map((r, ri) => {
        ...
        return <path key={ri} d={path} fill="none" stroke="rgba(0,51,153,0.15)" strokeWidth={1}/>;
      })}
      {skillResults.map((_, i) => {
        const [x,y] = point(i, R);
        return <line key={i} x1={cx} y1={cy} x2={x} y2={y} stroke="rgba(0,51,153,0.15)" strokeWidth={1}/>;
      })}
      <path d={dataPath} fill="rgba(0,51,153,0.15)" stroke="#003399" strokeWidth={2.5} strokeLinejoin="round"/>
      {dataPoints.map(([x,y], i) => (
        <circle key={i} cx={x} cy={y} r={5} fill={SKILL_COLORS[skillResults[i].code]} stroke="#fff" strokeWidth={2}/>
      ))}
      {rings.map((r, ri) => {
        const [x,y] = point(0, R*r);
        return <text key={ri} x={x+4} y={y-3} fontSize={9} fill="rgba(0,51,153,0.4)">{Math.round(r*100)}%</text>;
      })}
      {skillResults.map((s, i) => {
        ...
        return (
          <text key={i} x={x} y={y} textAnchor={anchor} fontSize={10} fontWeight={600} fill="#1a1a2e">
...
function generateReportHTML(candidateName, candidateCode, skillResults, date, consistency) {
  ...
  const svgSpider = `<svg width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}" xmlns="http://www.w3.org/2000/svg" style="overflow:visible">
    ...
    <path d="${dataPath}" fill="rgba(0,51,153,0.10)" stroke="#003399" stroke-width="3" stroke-linejoin="round"/>
  ...
  const tableRows = skillResults.map((s, rank) => `
    <tr style="background:${rank%2===0?"#F8FAFC":"#fff"}">
      ...
      <td style="padding:11px 14px;text-align:center">
        <span style="background:#EFF6FF;color:#003399;padding:3px 12px;border-radius:999px;font-weight:800;font-size:14px">${Math.round(s.ratio*100)}%</span>
      </td>
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .cover{background:linear-gradient(135deg,#003399 0%,#0050cc 60%,#002277 100%);padding:56px 64px;color:#fff;position:relative;overflow:hidden}
    .cover::after{content:'';position:absolute;top:-60px;right:-60px;width:280px;height:280px;background:rgba(255,255,255,0.06);border-radius:50%}
    .cover-badge{display:inline-block;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:#fff;padding:4px 16px;border-radius:999px;font-size:12px;letter-spacing:.1em;text-transform:uppercase;margin-bottom:28px}
    .cover-title{font-family:'Playfair Display',serif;font-size:38px;font-weight:700;line-height:1.2;margin-bottom:6px}
    .cover-sub{color:rgba(255,255,255,0.75);font-size:17px;font-style:italic;margin-bottom:44px}
    .cover-meta{display:grid;grid-template-columns:1fr 1fr;gap:20px}
    .meta-item label{color:rgba(255,255,255,0.6);font-size:11px;text-transform:uppercase;letter-spacing:.08em;display:block;margin-bottom:3px}
    .meta-item value{color:#fff;font-size:17px;font-weight:700;display:block}
    .section{padding:44px 64px;border-bottom:1px solid #F1F5F9}
    .section-title{font-family:'Playfair Display',serif;font-size:22px;color:#003399;margin-bottom:6px}
    ...
    thead th{background:#003399;color:#fff;padding:12px 14px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.07em}
    ...
    .print-btn{position:fixed;bottom:28px;right:28px;background:linear-gradient(135deg,#003399,#0050cc);color:#fff;border:none;border-radius:12px;padding:14px 28px;font-size:15px;font-weight:700;cursor:pointer;box-shadow:0 4px 20px rgba(0,51,153,0.35);font-family:'Source Sans 3',sans-serif}
    @media print{
      body{background:#fff}
...
<div class="page">
  ...
  ${(()=>{
    ...
    const components = [
      { name:"Σταθερότητα χαρακτηριστικών", weight:"40%", value:trait, definition:"Μετρά κατά πόσο ο υποψήφιος επιλέγει με συνέπεια δηλώσεις που αντιστοιχούν στις ίδιες δεξιότητες σε όλη τη διάρκεια της δοκιμασίας." },
      { name:"Συνοχή ζευγών δηλώσεων", weight:"40%", value:pair, definition:"Εξετάζει αν δηλώσεις που μετρούν την ίδια δεξιότητα και εμφανίζονται σε διαφορετικές τριάδες λαμβάνουν συνεπείς βαθμολογίες." },
      { name:"Μοτίβο απαντήσεων", weight:"20%", value:pattern, definition:"Ανιχνεύει μη ρεαλιστικά μοτίβα επιλογών, όπως συστηματική επιλογή κοινωνικά επιθυμητών δηλώσεων (faking good)." },
    ];
    return `<div class="section" style="border-left:5px solid ${borderCol};padding-left:59px;">
      ...
      <p class="section-sub">Αξιολογεί την αξιοπιστία και εσωτερική συνοχή των επιλογών του υποψηφίου.</p>
...
function WelcomeScreen({ data, onStart }) {
  ...
  const canStart = name.trim().length >= 2 && isValidAccessCode(code);

  function handleStart() {
    if (!isValidAccessCode(code)) {
      setCodeError("Μη έγκυρος κωδικός συμμετοχής. Παρακαλώ επαληθεύστε τον κωδικό σας.");
      return;
    }
    setCodeError("");
  ...
  return (
    <div style={styles.welcome}>
      <div style={styles.welcomeInner}>
        {/* ASEP Logo area */}
        <div style={styles.asepHeader}>
          <div style={styles.asepLogoBox}>
            <span style={styles.asepLogoText}>ΑΣΕΠ</span>
          </div>
          <div>
            <div style={styles.asepOrgName}>Ανώτατο Συμβούλιο Επιλογής Προσωπικού</div>
            <div style={styles.asepOrgSub}>Αξιολόγηση Εργασιακών Συμπεριφορών 2026</div>
          </div>
        </div>
        ...
        <div style={styles.candidateBox}>
          ...
          <div style={styles.fieldGroup}>
            <label style={styles.fieldLabel}>Όνομα / Nickname <span style={{color:"#cc0000"}}>*</span></label>
          ...
          <div style={{...styles.fieldGroup, marginBottom:0}}>
            <label style={styles.fieldLabel}>Κωδικός Συμμετοχής <span style={{color:"#cc0000"}}>*</span></label>
            <input
              style={{...styles.fieldInput, borderColor: codeError ? "#cc0000" : "rgba(0,51,153,0.25)"}}
              type="text"
              placeholder="ΑΣΕΠ Unlocked XX"
              value={code}
              onChange={e=>{ setCode(e.target.value); setCodeError(""); }}
              maxLength={20}
            />
            {codeError && (
              <div style={styles.codeError}>⚠️ {codeError}</div>
            )}
          </div>
        </div>
        ...
        <button
          style={{...styles.startBtn, opacity:canStart?1:0.45, cursor:canStart?"pointer":"not-allowed"}}
          onClick={handleStart}
          disabled={!canStart}
        >
          Έναρξη Δοκιμασίας →
        ...
        {!canStart && (
          <p style={{color:"#64748B",fontSize:13,marginTop:10,fontFamily:"sans-serif"}}>
            {name.trim().length < 2 ? "Εισάγετε τουλάχιστον 2 χαρακτήρες για το όνομα" : "Απαιτείται έγκυρος κωδικός συμμετοχής"}
          </p>
        )}
      </div>
    </div>
...
function TriadScreen({ data, currentIdx, answers, onAnswer, onPrev, onFinish, timerProps }) {
  ...
  return (
    // Full-screen overlay that covers everything including the iframe parent
    <div style={styles.fullScreenOverlay}>
      <div style={styles.triadScreen}>
        {/* Sticky header */}
        <div style={styles.header}>
          <div style={styles.asepHeaderSmall}>
            <span style={styles.asepLogoBadge}>ΑΣΕΠ</span>
            <span style={styles.asepTestTitle}>Δοκιμασία Εργασιακών Συμπεριφορών</span>
          </div>
            ...
            {triad.statements.map(stmt=>{
              ...
              return(
                <div key={stmt.id} style={{...styles.stmtCard,
                  ...
                  {label&&<span style={{...styles.rankBadge,background:state==="chosen-first"?"#10B981":state==="chosen-second"?"#003399":"#94A3B8"}}>{label}</span>}
                </div>
              );
            })}
          </div>
  ...
  return (
    <div style={styles.results}>
      <div style={styles.resultsInner}>
        <div style={styles.asepHeader}>
          <div style={styles.asepLogoBox}><span style={styles.asepLogoText}>ΑΣΕΠ</span></div>
          <div>
            <div style={{...styles.asepOrgName, color:"#003399"}}>Ανώτατο Συμβούλιο Επιλογής Προσωπικού</div>
            <div style={{...styles.asepOrgSub, color:"#64748B"}}>Αποτελέσματα Αξιολόγησης</div>
          </div>
        </div>

        <div style={{...styles.divider, borderColor:"#E2E8F0"}}/>
...
function TimeUpScreen({ onViewResults }) {
  return (
    <div style={{...styles.welcome,background:"linear-gradient(135deg,#7f1d1d 0%,#991b1b 50%,#7f1d1d 100%)"}}>
...
export default function App() {
  ...
  if(error)return<div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",color:"#cc0000",background:"#f8faff",fontFamily:"sans-serif",flexDirection:"column",gap:16}}><p style={{fontSize:18}}>⚠️ {error}</p></div>;
  if(screen==="loading")return<div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",color:"#003399",background:"#f8faff",fontFamily:"sans-serif"}}><p>⏳ Φόρτωση...</p></div>;
...
const styles = {
  // ── Welcome & Results shared ──
  welcome:{minHeight:"100vh",background:"linear-gradient(160deg,#f0f4ff 0%,#e8eeff 40%,#f5f7ff 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px",fontFamily:"Georgia,serif"},
  welcomeInner:{maxWidth:680,width:"100%",textAlign:"center"},

  // ASEP header
  asepHeader:{display:"flex",alignItems:"center",gap:16,marginBottom:24,textAlign:"left"},
  asepLogoBox:{background:"#003399",borderRadius:10,padding:"10px 16px",flexShrink:0},
  asepLogoText:{color:"#fff",fontSize:22,fontWeight:800,fontFamily:"sans-serif",letterSpacing:2},
  asepOrgName:{color:"#003399",fontSize:15,fontWeight:700,fontFamily:"sans-serif"},
  asepOrgSub:{color:"#64748B",fontSize:13,fontFamily:"sans-serif"},
  divider:{borderTop:"2px solid #003399",margin:"0 0 24px",opacity:0.15},

  badge:{display:"inline-block",background:"rgba(0,51,153,0.1)",border:"1px solid rgba(0,51,153,0.3)",color:"#003399",padding:"4px 16px",borderRadius:999,fontSize:13,letterSpacing:".1em",textTransform:"uppercase",marginBottom:24},
  heroTitle:{color:"#003399",fontSize:"clamp(22px,4vw,36px)",fontWeight:700,margin:"0 0 8px",lineHeight:1.2},
  heroSubtitle:{color:"#475569",fontSize:17,margin:"0 0 28px",fontStyle:"italic"},

  candidateBox:{background:"#fff",border:"2px solid rgba(0,51,153,0.15)",borderRadius:16,padding:"24px 28px",marginBottom:24,textAlign:"left",boxShadow:"0 2px 12px rgba(0,51,153,0.07)"},
  candidateTitle:{color:"#003399",fontSize:15,fontWeight:700,marginBottom:16,fontFamily:"sans-serif"},
  fieldGroup:{marginBottom:14},
  fieldLabel:{color:"#334155",fontSize:13,display:"block",marginBottom:6,fontFamily:"sans-serif",fontWeight:600},
  fieldInput:{width:"100%",background:"#F8FAFF",border:"1.5px solid rgba(0,51,153,0.25)",borderRadius:8,padding:"10px 14px",color:"#1E293B",fontSize:15,fontFamily:"sans-serif",outline:"none"},
  codeError:{color:"#cc0000",fontSize:13,marginTop:6,fontFamily:"sans-serif",textAlign:"left"},

  infoGrid:{display:"flex",gap:16,justifyContent:"center",marginBottom:24,flexWrap:"wrap"},
  infoCard:{background:"#fff",border:"1.5px solid rgba(0,51,153,0.15)",borderRadius:12,padding:"16px 24px",display:"flex",flexDirection:"column",alignItems:"center",gap:4,minWidth:120,boxShadow:"0 2px 8px rgba(0,51,153,0.06)"},
  infoIcon:{fontSize:24},
  infoLabel:{color:"#64748B",fontSize:12,textTransform:"uppercase",letterSpacing:".05em"},
  infoValue:{color:"#003399",fontSize:20,fontWeight:700},

  instructionsBox:{background:"#fff",border:"1.5px solid rgba(0,51,153,0.12)",borderRadius:16,padding:28,marginBottom:28,textAlign:"left",boxShadow:"0 2px 8px rgba(0,51,153,0.05)"},
  instrTitle:{color:"#003399",margin:"0 0 16px",fontSize:16,fontWeight:700,fontFamily:"sans-serif"},
  instrLine:{color:"#475569",fontSize:14,margin:"6px 0",display:"flex",gap:8,fontFamily:"sans-serif"},
  instrBullet:{color:"#003399",flexShrink:0,fontSize:16},

  startBtn:{background:"linear-gradient(135deg,#003399,#0050cc)",color:"#fff",border:"none",borderRadius:12,padding:"16px 48px",fontSize:18,fontWeight:700,cursor:"pointer",fontFamily:"sans-serif",boxShadow:"0 4px 24px rgba(0,51,153,0.3)"},

  // ── Full-screen test overlay ──
  fullScreenOverlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",zIndex:99999,background:"#F8FAFC",overflowY:"auto"},
  triadScreen:{minHeight:"100vh",background:"#F8FAFC",display:"flex",flexDirection:"column",fontFamily:"sans-serif"},

  // Sticky header inside test
  header:{background:"#fff",borderBottom:"2px solid #003399",padding:"10px 24px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10,flexWrap:"wrap",boxShadow:"0 2px 8px rgba(0,51,153,0.08)"},
  asepHeaderSmall:{display:"flex",alignItems:"center",gap:8,flexShrink:0},
  asepLogoBadge:{background:"#003399",color:"#fff",borderRadius:6,padding:"4px 10px",fontSize:13,fontWeight:800,fontFamily:"sans-serif",letterSpacing:1},
  asepTestTitle:{color:"#003399",fontSize:13,fontWeight:600,fontFamily:"sans-serif",display:"none"},
  triadBadge:{background:"#EFF6FF",color:"#003399",border:"1px solid rgba(0,51,153,0.2)",borderRadius:999,padding:"4px 12px",fontSize:13,fontWeight:700,flexShrink:0},
  progressBar:{flex:1,height:8,background:"#E2E8F0",borderRadius:999,overflow:"hidden",minWidth:60},
  progressFill:{height:"100%",background:"linear-gradient(90deg,#003399,#0050cc)",borderRadius:999,transition:"width 0.4s ease"},
  progressText:{color:"#003399",fontSize:13,fontWeight:700,flexShrink:0},

  testBody:{flex:1,display:"flex",flexDirection:"column",maxWidth:780,width:"100%",margin:"0 auto",padding:"0 16px"},
  stepBox:{margin:"20px 0 0",width:"100%"},
  stepText:{background:"#EFF6FF",border:"1px solid rgba(0,51,153,0.2)",borderRadius:10,padding:"12px 20px",color:"#003399",fontSize:15,margin:0},

  statements:{margin:"12px 0",width:"100%",display:"flex",flexDirection:"column",gap:12},
  ...
  stmtSecond:{border:"2px solid #003399",background:"#EFF6FF",boxShadow:"0 0 0 4px rgba(0,51,153,0.08)"},
  ...
  nav:{margin:"8px 0 32px",width:"100%",display:"flex",justifyContent:"space-between",gap:12},
  ...
  navBtnPrimary:{background:"linear-gradient(135deg,#003399,#0050cc)",color:"#fff",boxShadow:"0 4px 16px rgba(0,51,153,0.25)"},
  navBtnFinish:{background:"linear-gradient(135deg,#10B981,#059669)",color:"#fff",boxShadow:"0 4px 16px rgba(16,185,129,0.3)"},

  // ── Results ──
  results:{minHeight:"100vh",background:"linear-gradient(160deg,#f0f4ff 0%,#e8eeff 40%,#f5f7ff 100%)",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"40px 24px",fontFamily:"sans-serif"},
  resultsInner:{maxWidth:700,width:"100%"},
  resultsBadge:{display:"inline-block",background:"rgba(0,51,153,0.1)",border:"1px solid rgba(0,51,153,0.3)",color:"#003399",padding:"4px 16px",borderRadius:999,fontSize:13,letterSpacing:".1em",textTransform:"uppercase",marginBottom:16},
  resultsTitle:{color:"#003399",fontSize:36,fontWeight:700,margin:"0 0 8px",fontFamily:"Georgia,serif"},
  resultsSubtitle:{color:"#475569",fontSize:16,margin:"0 0 16px"},

  skillGrid:{display:"flex",flexDirection:"column",gap:10,marginBottom:28},
  skillRow:{display:"flex",alignItems:"center",gap:16,background:"#fff",border:"1.5px solid rgba(0,51,153,0.12)",borderRadius:12,padding:14,boxShadow:"0 2px 8px rgba(0,51,153,0.05)"},
  skillRankNum:{color:"#003399",fontSize:14,fontWeight:700,minWidth:32,flexShrink:0},
  skillInfo:{flex:1},
  skillHeader:{display:"flex",alignItems:"center",gap:8,marginBottom:6},
  skillName:{color:"#1E293B",fontSize:14,fontWeight:600,flex:1},
  skillPct:{color:"#003399",fontSize:14,fontWeight:700},
  skillBarBg:{height:7,background:"rgba(0,51,153,0.08)",borderRadius:999,overflow:"hidden"},
  skillBarFill:{height:"100%",borderRadius:999,transition:"width 0.8s cubic-bezier(0.4,0,0.2,1)"},

  reportBtn:{background:"linear-gradient(135deg,#003399,#0050cc)",border:"none",color:"#fff",borderRadius:10,padding:"13px 28px",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:"0 4px 16px rgba(0,51,153,0.25)"},
  restartBtn:{background:"#fff",border:"1.5px solid rgba(0,51,153,0.25)",color:"#003399",borderRadius:10,padding:"13px 28px",fontSize:15,fontWeight:600,cursor:"pointer"},
};

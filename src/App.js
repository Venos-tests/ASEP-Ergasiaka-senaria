...
function TriadScreen({ data, currentIdx, answers, onAnswer, onPrev, onFinish }) {
...
function ResultsScreen({ data, answers, onRestart }) {
  ...
  const skillResults = data.skills.map(s => ({
    ...
    percent: Math.round((scores[s.code].raw / (scores[s.code].appearances * 3)) * 100)
...
export default function App() {
  ...
  if (screen === "welcome") return <WelcomeScreen data={data} onStart={() => setScreen("test")} />;
  if (screen === "test") return <TriadScreen data={data} currentIdx={currentIdx} answers={answers} onAnswer={handleAnswer} onPrev={() => currentIdx > 0 && setCurrentIdx(currentIdx - 1)} onFinish={() => setScreen("results")} />;
  if (screen === "results") return <ResultsScreen data={data} answers={answers} onRestart={() => { setAnswers({}); setCurrentIdx(0); setScreen("welcome"); }} />;
}
...
const styles = {
  ...
  header: { background:"#fff", borderBottom:"1px solid #E2E8F0", padding:"12px 24px", display:"flex", alignItems:"center", gap:16, position:"sticky", top:0, zIndex:10 },
